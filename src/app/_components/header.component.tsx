  ...
  return (
    <div className=" justify-evenly flex md:justify-around items-center mb-10">
      <div className="flex items-center">
        ...
        <div className="pl-4 border-l">
         {status === "authenticated" ? (
            <ul>
              <Link href="/coaches" className="hidden md:block md:text-lg md:font-medium text-[#230E49] hover:opacity-40 hover:transition-opacity">coaches</Link>
            </ul>
         ): null}
        </div>
      </div>
      ...
      <div className="md:flex md:gap-6">
        ...
        {status !== "authenticated" ? (
          <Button className="bg-[#230E49] hover:bg-indigo-950"
        ...
        <>
          {session?.user?.Role === "COACH" ? (
            ...
            onClick={() => router.push(`/user/${session.user.id}/coachings/create`)}
            >create new Post
              <PiPlusCircleBold/>
            </Button>
            </>
